...
export function ProcessingOverlay({
  ...
  const determinate = !isError && progress !== null && progress >= 0;
  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col items-center justify-center gap-4 bg-[#080b10]/88 px-6 backdrop-blur-md">
      <div className="relative h-[4.5rem] w-[4.5rem]">
        {isError ? (
          <div className="absolute inset-0 flex items-center justify-center rounded-full border-2 border-red-500/40 bg-red-500/15">
            <AlertCircle className="h-10 w-10 text-red-400" strokeWidth={1.75} />
          </div>
        ) : (
          <>
      ...
      <p
        className={cn(
          "max-w-md text-center text-sm font-semibold leading-snug",
      ...
      {isError && onDismissError && (
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-red-500/50 bg-red-500/15 px-4 py-2 text-xs font-bold text-red-100 hover:bg-red-500/25"
        >
          <X className="h-4 w-4" />
          Închide
        </button>
